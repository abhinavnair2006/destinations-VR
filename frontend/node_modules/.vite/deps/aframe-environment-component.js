// node_modules/aframe-environment-component/index.js
...
AFRAME.registerComponent("environment", {
  schema: {
    ...
    seed: { type: "int", default: 1, min: 0, max: 1e3 },
    ...
    stageSize: { type: "number", default: 200, min: 1, max: 2e4 },
  ...
  init: function() {
    ...
    this.assets = {
      ...
      "towers": [
        { type: "extrude", vertices: [-0.054, -0.178, -7e-3, -0.182, 0.069, -0.027, 0.189, 0.079, 0.178, 0.124, -7e-3, 0.097, -0.145, 0.182, -0.178, 0.144, -0.079, -0.021] },
        { type: "lathe", segments: 4, vertices: [4e-3, 0.02, 0.012, 0.092, 0.042, 0.166, 0.067, 0.55, 0.101, 0.594, 0.105, 0.838, 0.193, 0.934, 0.18, 0.994] },
        { type: "lathe", segments: 5, vertices: [0.069, 0.216, 0.067, 0.562, 0.126, 0.562, 0.128, 0.774, 0.191, 0.774, 0.193, 0.986] }
      ],
      "trees": [
        { type: "lathe", noise: 0.015, segments: 6, vertices: [1e-6, 0.826, 0.054, 0.832, 0.105, 0.854, 0.136, 0.9, 0.136, 0.958, 0.118, 0.994] },
        { type: "lathe", noise: 0.015, segments: 14, vertices: [1e-6, 0.01, 0.069, 0.022, 0.13, 0.068, 0.178, 0.18, 0.189, 0.32, 0.191, 0.59, 0.193, 0.75, 0.138, 0.79, 0.018, 0.808, 0.018, 0.996] },
        { type: "lathe", noise: 0.015, segments: 14, vertices: [1e-6, 0.436, 0.126, 0.46, 0.201, 0.57, 0.219, 0.72, 0.154, 0.846, 0.028, 0.884, 0.034, 0.996] }
      ],
      "apparatus": [
        { type: "lathe", segments: 10, vertices: [1e-6, 0.23, 0.042, 0.23, 0.069, 0.36, 0.038, 0.362, 0.038, 0.372, 0.06, 0.372, 0.073, 0.572, 0.024, 0.572, 0.024, 0.67, 0.069, 0.67, 0.075, 0.722, 0.097, 0.724, 0.105, 0.852, 0.083, 0.902, 0.065, 0.902, 0.065, 0.924, 0.128, 0.924, 0.146, 0.996] },
        { type: "lathe", segments: 16, vertices: [1e-6, 0.232, 0.229, 0.182, 0.486, 0.07, 0.356, 0.182, 0.213, 0.242, 0.154, 0.242, 0.144, 0.262, 0.178, 0.262, 0.126, 0.314, 0.04, 0.328, 0.038, 0.374, 0.058, 0.374, 0.071, 0.408, 0.026, 0.406, 0.03, 0.42, 0.091, 0.418, 0.034, 0.496, 0.01, 0.498, 0.03, 0.506, 0.014, 0.998] }
      ],
      "mushrooms": [
        { type: "lathe", noise: 0.02, segments: 14, vertices: [1e-6, 6e-3, 0.13, 0.018, 0.341, 0.084, 0.437, 0.144, 0.492, 0.234, 0.484, 0.246, 0.276, 0.232, 0.107, 0.284, 0.046, 0.346, 0.062, 0.852, 0.097, 0.956, 0.166, 0.998] },
        { type: "lathe", noise: 0.02, segments: 10, vertices: [1e-6, 0.562, 0.091, 0.572, 0.172, 0.61, 0.223, 0.666, 0.256, 0.74, 0.258, 0.806, 0.246, 0.824, 0.062, 0.826, 0.065, 0.948, 0.097, 0.998] },
        { type: "lathe", noise: 0.02, segments: 10, vertices: [1e-6, 0.768, 0.099, 0.772, 0.219, 0.802, 0.306, 0.844, 0.352, 0.886, 0.352, 0.908, 0.118, 0.904, 0.107, 0.93, 0.115, 0.966, 0.14, 0.996] }
      ]
    };
  ...
  update: function(oldDataNonPreset) {
    ...
    if (skyType !== oldData.skyType || this.environmentData.skyColor != oldData.skyColor || this.environmentData.horizonColor != oldData.horizonColor) {
      var mat = {};
    ...
    if (skyType == "atmosphere") {
      ...
      this.setStars((1 - Math.max(0, (sunPos.y + 0.08) * 8)) * 2e3);
    }
    ...
    var updateGroundGeometry = !this.groundGeometry || this.environmentData.seed != oldData.seed || this.environmentData.ground != oldData.ground || this.environmentData.playArea != oldData.playArea || this.environmentData.flatShading != oldData.flatShading || this.environmentData.groundDensity != oldData.groundDensity || this.environmentData.groundFrequency != oldData.groundFrequency || updateStageSize;
    if (updateGroundGeometry || this.environmentData.groundColor != oldData.groundColor || this.environmentData.groundColor2 != oldData.groundColor2 || this.environmentData.groundYScale != oldData.groundYScale || this.environmentData.groundTexture != oldData.groundTexture || this.environmentData.gridColor != oldData.gridColor || this.environmentData.grid != oldData.grid) {
    ...
    if (this.environmentData.seed != oldData.seed || this.environmentData.dressingOnPlayArea != oldData.dressingOnPlayArea || this.environmentData.dressing != oldData.dressing || this.environmentData.flatShading != oldData.flatShading || this.environmentData.dressingAmount != oldData.dressingAmount || this.environmentData.dressingScale != oldData.dressingScale || this.environmentData.dressingColor != oldData.dressingColor || this.environmentData.dressingVariance.x != oldData.dressingVariance.x || this.environmentData.dressingVariance.y != oldData.dressingVariance.y || this.environmentData.dressingVariance.z != oldData.dressingVariance.z || this.environmentData.dressingUniformScale != oldData.dressingUniformScale || updateStageSize) {
      this.updateDressing();
    }
  ...
  dumpParametersDiff: function() {
    ...
    for (var i in this.schema) {
      if (i == "preset" || usingPreset && usingPreset[i] === void 0) {
        continue;
      }
  ...
  updateGround: function(updateGeometry) {
    var resolution = this.environmentData.groundDensity;
    if (updateGeometry) {
  ...
  getAssetGeometry: function(data) {
    ...
    var geoset = [];
    var self = this;
    function applyNoise(geo2, noise) {
      var verts2 = geo2.attributes.position.array;
      var numVerts = verts2.length;
      for (var i2 = 0; i2 < numVerts; i2 += 3) {
        verts2[i2] = (self.random(i2) - 0.5) * noise;
        verts2[i2 + 1] = (self.random(i2 + numVerts) - 0.5) * noise;
        verts2[i2 + 2] = (self.random(i2 + numVerts * 2) - 0.5) * noise;
      }
  ...
  createStars: function() {
    ...
    var material = new THREE.PointsMaterial({ size: 0.01, color: 13421772, fog: false });
    this.rendererSystem.applyColorCorrection(material.color);
  ...
  setStars: function(numStars) {
    numStars = Math.floor(Math.min(2e3, Math.max(0, numStars)));
    if (numStars === 0) {
      this.removeStars();
    } else {
      if (!this.stars) {
...
AFRAME.registerShader("skyshader", {
  schema: {
    exposureBias: { type: "number", default: 1, min: 0, max: 10, is: "uniform" },
    turbidity: { type: "number", default: 2, min: 0, max: 20, is: "uniform" },
    reileigh: { type: "number", default: 1, min: 0, max: 4, is: "uniform" },
    mieCoefficient: { type: "number", default: 5e-3, min: 0, max: 0.1, is: "uniform" },
...
//# sourceMappingURL=aframe-environment-component.js.map
